...
export default function MenuExA() {
  ...
  const clickMe = (e: React.MouseEvent<HTMLSpanElement, MouseEvent>) => {
    const { current } = ref;
    let alt = e.currentTarget;
    let num: number;
    if (typeof alt.dataset.badges === "string") {
      num = parseInt(alt.dataset.badges);
    }
    setLetter((letter) => num);
  ...
  return (
    <div className="one">
      ...
      <div className="menu">
        <span data-badges={0} onClick={(e) => clickMe(e)}>
          {svg[0]}
        </span>
        <span data-badges={1} onClick={(e) => clickMe(e)}>
          {svg[1]}
        </span>
        <span data-badges={2} onClick={(e) => clickMe(e)}>
          {svg[2]}
        </span>
        <span data-badges={3} onClick={(e) => clickMe(e)}>
          {svg[3]}
        </span>
      </div>
    </div>
